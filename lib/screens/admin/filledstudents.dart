...
class _studentpage2State extends State<studentpage2> {
  ...
  fetch_all_data() async {
    ...
    try {
      ...
      for (var i in studentdocs) {
        p.add(i["filledstudents"]);
        e.add(i['initialshortlistedstudents']);
      }
      for (var i in p[0]) {
  ...
  updatedata2(var text1) async {
    ...
    leaves.doc(l[0]).update({
      'initialshortlistedstudents': FieldValue.arrayUnion([text1]),
    });
  }
}
